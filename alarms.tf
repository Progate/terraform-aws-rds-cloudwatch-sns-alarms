...
module "burst_balance_too_low_label" {
  ...
  stage      = var.stage
  attributes = compact(concat(var.attributes, ["burst", "balance", "too", "low"]))
}
...
module "cpu_utilization_too_high_label" {
  ...
  stage     = var.stage
  attributes = compact(
    concat(var.attributes, ["cpu", "utilization", "too", "high"]),
  )
}
...
module "cpu_credit_balance_too_low_label" {
  ...
  stage     = var.stage
  attributes = compact(
    concat(var.attributes, ["cpu", "credit", "balance", "too", "low"]),
  )
}
...
module "disk_queue_depth_too_high_label" {
  ...
  stage     = var.stage
  attributes = compact(
    concat(var.attributes, ["disk", "queue", "depth", "too", "high"]),
  )
}
...
module "freeable_memory_too_low_label" {
  ...
  namespace  = var.namespace
  stage      = var.stage
  attributes = compact(concat(var.attributes, ["freeable", "memory", "too", "low"]))
}
...
module "free_storage_space_threshold_label" {
  ...
  stage     = var.stage
  attributes = compact(
    concat(var.attributes, ["free", "storage", "space", "threshold"]),
  )
}
...
module "swap_usage_too_high_label" {
  ...
  namespace  = var.namespace
  stage      = var.stage
  attributes = compact(concat(var.attributes, ["swap", "usage", "too", "high"]))
}
